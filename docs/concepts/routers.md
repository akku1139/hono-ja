...
しかしながら、RegExpRouter はすべてのルーティングパターンをサポートしているわけではありません。そのため通常は、以下のルータを組み合わせて使用し、すべてのルーティングパターンをサポートします。
...
**SmartRouter** は、複数のルータを使用する際に役に立ちます。登録済みのルータから最もよいルータを推測して選択します。
Hono はデフォルトでは、SmartRouter, RegExpRouter, TrieRouter を使用します。
