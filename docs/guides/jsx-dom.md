...
hono/jsx/dom にはフックがあります。これらは、React と完全に互換性があるか、または部分的に互換性があります。[the React documentation](https://react.dev/reference/react/hooks) を参照してこれらの API を学ぶことができます。
...
あるいは、`vite.config.ts` で esbuild 変換オプション内で `hono/jsx/dom` を指定することができます。
